...
NSString *LGGlassMetalSource(void) {
    static NSString * const source = @"// fullscreen quad + glass shading\n"
        ...
        "    float highlight = specular + fresnel * 0.42;\n"
        "    float3 lch = srgbToLch(clamp(bgColor.rgb, 0.0, 1.0));\n"
        "    lch.x = clamp(lch.x + highlight * 26.0, 0.0, 100.0);\n"
        "    lch.y = max(0.0, lch.y - highlight * 6.5);\n"
        "    float3 shapedHighlight = lchToSrgb(lch);\n"
        "    bgColor.rgb = mix(bgColor.rgb, shapedHighlight, clamp(highlight * 0.55, 0.0, 1.0));\n"
